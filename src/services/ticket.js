...
/** Conserva todos los embeds del mensaje principal (formato + ticket) al editar. */
function rebuildTicketEmbeds(ticketMessage, updateFirstEmbedFn) {
  return ticketMessage.embeds.map((e, idx) => {
    const builder = EmbedBuilder.from(e);
    if (idx === 0 && typeof updateFirstEmbedFn === 'function') {
      updateFirstEmbedFn(builder, e);
    }
    return builder;
  });
}
  ...
  try {
    ...
    if (ticketMessage) {
      const embeds = rebuildTicketEmbeds(ticketMessage, (builder, e) => {
        const fields = (e.fields || []).map((f) => ({ name: f.name, value: f.value, inline: f.inline ?? false }));
        const statusField = fields.find((f) => f.name === 'Estado' || f.name === 'Status');
        if (statusField) statusField.value = '🔴 Cerrado';
        builder.setFields(fields);
        builder.setColor('#e74c3c');
      });
      await ticketMessage.edit({ embeds, components: [] });
    }
...
export async function claimTicket(channel, claimer) {
  try {
    ...
    if (ticketMessage) {
      ...
      const embeds = rebuildTicketEmbeds(ticketMessage, (builder, e) => {
        const fields = (e.fields || []).map((f) => ({ name: f.name, value: f.value, inline: f.inline ?? false }));
        const claimedField = fields.find((f) => f.name === 'Reclamado por' || f.name === 'Claimed By');
        if (claimedField) claimedField.value = claimer.toString();
        builder.setFields(fields);
      });
      await ticketMessage.edit({ embeds, components: [row] });
    }
...
export async function unclaimTicket(channel, unclaimer) {
  try {
    ...
    if (ticketData.claimedBy !== unclaimer.id && !(unclaimer.permissions && unclaimer.permissions.has && unclaimer.permissions.has(PermissionFlagsBits.ManageChannels))) {
