  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      'Elegí una **categoría** abajo y después el ítem que quieras comprar.\n\n' +
        ...
        '• Permisos y seguros también aparecen en el inventario.'
    )
...
const MENSAJES_FUMAR = {
  pack_newport: `${E.cigarro || ''} Encendiste un Newport. Break rápido antes de volver a la sesión.`,
  shot_henny: `${E.cigarro || ''} Un shot de Henny. La garganta arde, el RP sigue.`,
  joint: `${E.cigarro || ''} Te armaste el joint y bajaste un cambio. Tranqui en el server.`
};
...
export default {
  ...
  async execute(interaction) {
    ...
    if (sub === 'inventario') {
      ...
      const { texto } = await armarInventarioCompleto(user.id, member);
    ...
    if (sub === 'comer') {
      ...
      const msg = MENSAJES_COMER[itemId] || `${E.comida || ''} Consumiste **${res.item.name}**.`;
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle('Comida')
    ...
    if (sub === 'fumar') {
      ...
      const msg = MENSAJES_FUMAR[itemId] || `${E.cigarro || ''} Usaste **${res.item.name}**.`;
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
    ...
    if (sub === 'regalar') {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle('Regalo')
            .setDescription(`${E.rosa || ''} ${interaction.user} le regaló una **${res.item.name}** a ${target}.`)
            .setColor(TIENDA_COLOR)
        ]
      });
    }
