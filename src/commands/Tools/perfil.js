    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (respuestaBloxlink.ok) {
                ...
                if (datosBloxlink.robloxID) robloxId = datosBloxlink.robloxID;
            }
        } catch (error) {
        ...
        recolector.on('collect', async (botonInteraction) => {
            if (botonInteraction.customId.startsWith('invtienda_')) {
                ...
                let member = null;
                try { member = await botonInteraction.guild.members.fetch(targetId); } catch (_) {}
                const { texto } = await armarInventarioCompleto(targetId, member);
