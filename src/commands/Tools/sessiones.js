...
export default {
    data: {
        name: 'inicio',
        description: 'Lanza un inicio de sesion de Roleplay o Car Meet para SWFL.',
        options: [
            {
                name: 'tipo',
                description: 'Tipo de sesion a iniciar',
    ...
    async execute(interaction) {
        if (!interaction.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
            return await interaction.reply({
                content: 'Solo el Staff autorizado puede iniciar sesiones.',
                ephemeral: true
            });
        }
        ...
        if (!check.ok) {
            if (check.razon === 'blacklist') {
                return interaction.reply({
                    content: 'Estas en la blacklist permanente de sesiones. No puedes iniciar ni participar.',
                    ephemeral: true
                });
            }
            const hasta = check.hasta ? `<t:${Math.floor(new Date(check.hasta).getTime() / 1000)}:R>` : 'pronto';
            return interaction.reply({
                content: `Estas suspendido de sesiones hasta ${hasta}. Motivo: ${check.motivo || '-'}`,
                ephemeral: true
            });
        }
        ...
        const esRP = tipo === 'rp';
        const titulo = esRP
            ? '<a:mari:1534954231138746488> Southwest Florida - Roleplay Sesion Inicio <a:mari:1534954231138746488>'
            : '<a:mari:1534954231138746488> Southwest Florida - Car Meet Sesion Inicio <a:mari:1534954231138746488>';

        const descExtra = esRP
            ? `> <:dot:1534938142665084938> Registra tus vehiculos en <#1505615426305130657>!\n\n`
            : `> <:dot:1534938142665084938> Recuerda evitar colisiones con vehiculos y mantener el realismo!\n\n`;

        const embed = new EmbedBuilder()
            .setTitle(titulo)
            .setDescription(
                `> ${ePunto} <@${interaction.user.id}> esta organizando una **sesion de ${esRP ? 'roleplay' : 'car meet oficial'}**! Antes de unirte, asegurate de que la configuracion de privacidad de tu cuenta este en **Everyone**. Al unirte, confirmas que leiste las normas del servidor.\n\n` +
                    `**Antes de Unirte**\n\n` +
                    `> <:dot:1534938142665084938> Asegurate de estar verificado.\n` +
                    `> <:dot:1534938142665084938> Lee la informacion y la lista de vehiculos baneados.\n` +
                    descExtra +
                    `> <:replica:1534982812116062370> El host debe obtener **${reacciones}+** reacciones antes de comenzar.`
            )
            .setColor('#74d4fc');
        ...
        try {
            ...
            console.error('Error al agregar reaccion inicial:', e);
        }
