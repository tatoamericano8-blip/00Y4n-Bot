...
export default {
    ...
    async execute(interaction) {
        ...
        // Generar timestamp dinámico de Discord
        const timestampDiscord = Math.floor(ahora.getTime() / 1000);

        // Diseñar el Embed Inicial de Solicitud (Estilo 00Y4n)
        ...
        // ⏱️ COLECTOR AUTOMÁTICO DE REACCIONES
        const collector = mensajeEnviado.createReactionCollector({
            filter: (reaction, user) => !user.bot,
            time: 14400000 // Escucha activa durante 4 horas
        });

        collector.on('collect', async (reaction) => {
            try {
                // Obtener lista actualizada de usuarios reales
        ...
        collector.on('end', async (_, reason) => {
            if (reason === 'meta_alcanzada') {
                // 1. Eliminar el Embed de solicitud original
                try {
                    await mensajeEnviado.delete();
                } catch (error) {
                    console.error('Error al eliminar el mensaje original de reinvitación:', error);
                }

                // 2. Tiempos para la versión liberada
                ...
                // 3. Crear el Embed de Liberación (Estilo unificado /lanzar_rp y /lanzar_meet)
                const infoDescripcion = 
                    `> <:dot:1523041306836996156> <@${interaction.user.id}> **¡ha liberado las reinvitaciones de la sesión!** Se ha alcanzado la meta de reacciones requeridas. Podés unirte al servidor utilizando los botones de abajo.\n\n` +
                ...
                // 4. Filas de Botones: Botón del Bot + Botón directo de Roblox
                const fila = new ActionRowBuilder().addComponents(
                    ...
                    new ButtonBuilder()
                        .setLabel('Unirse a Roblox')
                        .setStyle(ButtonStyle.Link)
                        .setURL(linkSesion)
                );

                // 5. Publicar el nuevo anuncio oficial con ping @everyone
                ...
                // 6. Guardar en memoria global.coleccionSesiones para compatibilidad total
