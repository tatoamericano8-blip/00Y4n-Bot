...
import { initializeDatabase, getFromDb } from './utils/database.js';
...
class TitanBot extends Client {
  ...
  setupOportunidades() {
    ...
    const MIN_MIN = 60;
    const MAX_MIN = 180;
    const programarSiguiente = async (esPrimera = false) => {
      let minutos = Math.floor(Math.random() * (MAX_MIN - MIN_MIN + 1)) + MIN_MIN;
      if (esPrimera) {
        try {
          const last = Number(await getFromDb('oportunidad:lastLaunch', 0)) || 0;
          const haceMin = last ? (Date.now() - last) / 60000 : 9999;
          if (haceMin >= MAX_MIN) {
            minutos = Math.floor(Math.random() * (12 - 3 + 1)) + 3;
            startupLog(`[oportunidad] Última hace ${Math.floor(haceMin)} min → próxima en ~${minutos} min.`);
          } else {
            const restante = Math.max(5, Math.ceil(MAX_MIN - haceMin));
            minutos = Math.min(restante, MAX_MIN);
            startupLog(`[oportunidad] Última hace ${Math.floor(haceMin)} min → próxima en ~${minutos} min.`);
          }
        } catch (_) {
          minutos = Math.floor(Math.random() * (15 - 5 + 1)) + 5;
          startupLog(`[oportunidad] Sin historial → primera en ~${minutos} min.`);
        }
      } else {
        startupLog(`[oportunidad] Próxima programada en ~${minutos} min.`);
      }
      setTimeout(async () => {
        try {
          await lanzarOportunidadEconomica(this, CANAL_GENERAL_ID);
        } catch (e) {
          logger.error('[oportunidad] Falló el envío programado:', e);
        }
